...
namespace DemoObserver
{
    ...
    public class EventDispatcher : MonoBehaviour
    {
        ...
        #region Fields
        /// Store all "listener"
        Dictionary<sEventID, Action<object>> _listeners = new Dictionary<sEventID, Action<object>>();
        #endregion
        ...
        public void RegisterListener(sEventID eventID, Action<object> callback)
        {
            // checking params
            Common.Assert(callback != null, "AddListener, event {0}, callback = null !!", eventID.ToString());
            Common.Assert(eventID != sEventID.None, "RegisterListener, event = None !!");
        ...
        public void PostEvent(sEventID eventID, object param = null)
        {
            if (!_listeners.ContainsKey(eventID))
            {
        ...
        public void RemoveListener(sEventID eventID, Action<object> callback)
        {
            // checking params
            Common.Assert(callback != null, "RemoveListener, event {0}, callback = null !!", eventID.ToString());
            Common.Assert(eventID != sEventID.None, "AddListener, event = None !!");
    ...
    public static class EventDispatcherExtension
    {
        ...
        public static void RegisterListener(this MonoBehaviour listener, sEventID eventID, Action<object> callback)
        {
            EventDispatcher.Instance.RegisterListener(eventID, callback);
        }
        public static void RemoveListenner(this MonoBehaviour listener, sEventID eventID, Action<object> callback)
        {
            EventDispatcher.Instance.RemoveListener(eventID, callback);
        }
        /// Post event with param
        public static void PostEvent(this MonoBehaviour listener, sEventID eventID, object param)
        {
            EventDispatcher.Instance.PostEvent(eventID, param);
        }

        /// Post event with no param (param = null)
        public static void PostEvent(this MonoBehaviour sender, sEventID eventID)
        {
